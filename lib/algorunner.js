...
class Algorunner {
    ...
    _registerToCommunicationEvents() {
        ...
        workerCommunication.on(messages.incomming.initialize, (data) => {
            ...
            this._code = data.extraData && data.extraData.code && data.extraData.code.join('\n');
        ...
        workerCommunication.on(messages.incomming.start, async (data) => {
            log.info(`running ${this._code} with input: ${JSON.stringify(this._input)}`);
            workerCommunication.send({
                command: messages.outgoing.started
            });
            try {
                const wrapper = (input, stopEmitter) => {
                    return new Promise((resolve, reject) => {
                        ...
                        const userFunctionPromise = Promise.resolve(vm.runInThisContext(`(${this._code})`)(input));
                ...
                let output = await wrapper(this._input, this._stopEmitter);
