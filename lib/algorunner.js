...
class Algorunner {
    ...
    */ 
    ...
    _registerToCommunicationEvents() {
        ...
        workerCommunication.on(messages.incomming.startedSubPipline, (data) => {
        ...
        workerCommunication.on(messages.incomming.subPipelineDone, (data) => {
            if (data && data.subPipelineId && data.response && (data.response instanceof Array)) {
                ...
                log.debug(`got subpipeline ${data.subPipelineId} buggy done command, sending error`);
                this._sendError()
                workerCommunication.send(new Error(`SubPipelineError - subPipelineId=${data.subPipelineId}: got buggy done command`));
            }
        });

        // handle subpipeline error event
        workerCommunication.on(messages.incomming.subPipelineError, (data) => {
            log.debug(`got subpipeline ${data.subPipelineId} error command: ${data.error}`);
            this._sendError(new Error(`SubPipelineError - subPipelineId=${data.subPipelineId}: ${data.error}`));
        });
