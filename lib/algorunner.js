...
class Algorunner {
    ...
    _registerToCommunicationEvents() {
        ...
        workerCommunication.on(messages.incomming.initialize, (data) => {
            ...
            log.info(`got 'initialize' command with date: ${JSON.stringify(data)}`);
            console.log(`got 'initialize' command with date: ${JSON.stringify(data)}`)
        });
        ...
        workerCommunication.on(messages.incomming.start, (data) => {
            ...
            try {
                ...
                let output = userFunction(this._input.slice(1),require);
