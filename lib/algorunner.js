...
class Algorunner {
    ...
    _registerToCommunicationEvents() {
        ...
        workerCommunication.on(messages.incoming.subPipelineStarted, (data) => {
            ...
            log.debug(`subpipeline ${subPipelineId} started`);
        ...
        workerCommunication.on(messages.incoming.subPipelineStopped, (data) => {
            let subPipelineId = data && data.subPipelineId;
            log.debug(`got subPipeline ${subPipelineId} stopped`);
    ...
    _initialize(data) {
        ...
        log.debug(`got 'initialize' command`);
    }

    async _start() {
        let stopped = false;
        log.debug(`running with input: ${this._input.length}`);
        workerCommunication.send({
