...
class Algorunner {
    ...
    */
     ...
     * - input
     */
    startAlgorithm({ execId, algorithmName, input, resultAsRaw }) {
        const command = messages.outgoing.startAlgorithmExecution;
        return this._startInstance({ type: TYPES.algorithms, command, data: { execId, algorithmName, input, resultAsRaw }, idPath: 'execId' });
    }
    ...
    _onAlgorithmError(data) {
        this._onInstanceError({ id: data.execId, type: TYPES.algorithms, error: data.error });
    }
    ...
    _checkAllInstancesResult() {
        ...
        Object.entries(this._instanceIds).forEach(([k, v]) => {
            if (this._isAllInstancesDone(k)) {
                results.push(...Object.values(v).map(r => r.response));
            }
            else {
                isDone = false;
            }
