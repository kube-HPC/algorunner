...
class Algorunner {
    ...
    async init(options) {
        ...
        if (options.capabilities) {
            this._url += `?${querystring.stringify(options.capabilities)}`;
        }
        console.debug(`connecting to ${this._url}`);
        this._wsc = new Websocket({ url: this._url, encoding, ...options });
    ...
    _checkAllInstancesResult() {
        ...
        Object.entries(this._instanceIds).forEach(([k, v]) => {
            if (this._isAllInstancesDone(k)) {
                results.push(...Object.values(v).map(r => r.response || r.error.message));
